...
public class EmergencyActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        locationSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                updateCampusMapImage(selectedLocation);
            }
    ...
    private void updateCampusMapImage(String locationName) {
        // Convert location name to a valid drawable resource name
        String resourceName = "map_" + locationName
                .toLowerCase()          // Lowercase
                .replace(" ", "_")       // Replace spaces with underscores
                .replace("-", "_");      // Replace hyphens if necessary

        int imageResId = getResources().getIdentifier(resourceName, "drawable", getPackageName());

        if (imageResId != 0) {
            campusMapImageView.setImageResource(imageResId);
        } else {
            // If no matching image found, set a default or clear
            campusMapImageView.setImageResource(R.drawable.ic_launcher_foreground);
        }
    }
